...
public class CoordinateManager : MonoBehaviour {
	...
	public void CreatePoints (float dis, int size)
	{
		...
		for (int x=0; x<size; x++ )
		{
			for (int z=0; z<size; z++)
			{
				...
				newPoint.zPos = startPoint - (z * dis);
				newPoint.x = x + 1;
				newPoint.z = z + 1;
	...
	public string ClickToCoordinate ( Vector3 pos )
	{
		...
		//float boardSize = GameObject.Find ("_GameSettings").GetComponent<GameSettings> ().game.boardSize;
		...
		for (int i=0; i<(letters.Count*2); i++)
		{
			for (int g=0; g<maxSize;g++)
			{
				if (i < letters.Count) {
					...
					string coor = letters [i] + letters[i] + g.ToString ();
					if (coor == coordinate) {
						index = i;
					}
				}
